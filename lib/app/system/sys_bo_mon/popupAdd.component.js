import { __decorate, __metadata, __param } from "tslib";
import { MatDialog, MatDialogRef, MAT_DIALOG_DATA, } from '@angular/material/dialog';
import { Component, Inject } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { sys_bo_mon_service } from '../../service/sys_bo_mon.service';
import Swal from 'sweetalert2';
import { sys_bo_mon_model } from '@/app/model/sys_bo_mon.model';
import { sys_user_service } from '../../service/sys_user.service';
let sys_bo_mon_popupComponent = class sys_bo_mon_popupComponent {
    http;
    sys_bo_mon_service;
    dialog;
    sys_user_service;
    dialogRef;
    data;
    sys_bo_mon_model = new sys_bo_mon_model();
    lst_status = [];
    check_error = [];
    id_user;
    constructor(http, sys_bo_mon_service, dialog, sys_user_service, dialogRef, data) {
        this.http = http;
        this.sys_bo_mon_service = sys_bo_mon_service;
        this.dialog = dialog;
        this.sys_user_service = sys_user_service;
        this.dialogRef = dialogRef;
        this.data = data;
        this.sys_bo_mon_model = data;
        this.get_profile_user();
    }
    get_profile_user() {
        this.sys_user_service.get_profile_user().subscribe((res) => {
            this.id_user = res.id;
            if (this.sys_bo_mon_model.db.id == 0) {
                this.Save();
                this.sys_bo_mon_model.db.create_by = this.id_user;
            }
            else {
                this.sys_bo_mon_model.db.update_by = this.id_user;
            }
        }, (err) => {
            console.log(err);
        });
    }
    Close() {
        this.dialogRef.close();
    }
    Save() {
        this.sys_bo_mon_service.add(this.sys_bo_mon_model).subscribe((result) => {
            var data = result;
            this.check_error = data.error;
            if (this.check_error.length === 0) {
                this.Close();
                Swal.fire({
                    icon: 'success',
                    title: 'Thành công',
                    showConfirmButton: false,
                    timer: 2000,
                }).then((result) => { });
            }
        });
    }
    Edit() {
        this.sys_bo_mon_service.edit(this.sys_bo_mon_model).subscribe((result) => {
            if (this.check_error.length === 0) {
                this.Close();
                Swal.fire({
                    icon: 'success',
                    title: 'Thành công',
                    showConfirmButton: false,
                    timer: 2000,
                }).then((result) => { });
            }
        });
    }
    ngOnInit() {
        this.lst_status = [
            {
                id: '1',
                name: 'Đang sử dụng',
            },
            {
                id: '2',
                name: 'Ngưng sử dụng',
            },
        ];
    }
};
sys_bo_mon_popupComponent = __decorate([
    Component({
        selector: 'sys_bo_mon_popup',
        templateUrl: './popupAdd.component.html',
        styleUrls: ['./popupAdd.component.scss'],
    }),
    __param(5, Inject(MAT_DIALOG_DATA)),
    __metadata("design:paramtypes", [HttpClient,
        sys_bo_mon_service,
        MatDialog,
        sys_user_service,
        MatDialogRef,
        sys_bo_mon_model])
], sys_bo_mon_popupComponent);
export { sys_bo_mon_popupComponent };
//# sourceMappingURL=popupAdd.component.js.map