import { __decorate, __metadata, __param } from "tslib";
import { MatDialog, MatDialogRef, MAT_DIALOG_DATA, } from '@angular/material/dialog';
import { Component, Inject } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { sys_chuyen_nganh_service } from '../../service/sys_chuyen_nganh.service';
import Swal from 'sweetalert2';
import { sys_user_service } from '../../service/sys_user.service';
import { sys_chuyen_nganh_model } from '@/app/model/sys_chuyen_nganh.model';
let sys_chuyen_nganh_popupComponent = class sys_chuyen_nganh_popupComponent {
    http;
    sys_chuyen_nganh_service;
    dialog;
    sys_user_service;
    dialogRef;
    data;
    sys_chuyen_nganh_model = new sys_chuyen_nganh_model();
    lst_status = [];
    check_error = [];
    constructor(http, sys_chuyen_nganh_service, dialog, sys_user_service, dialogRef, data) {
        this.http = http;
        this.sys_chuyen_nganh_service = sys_chuyen_nganh_service;
        this.dialog = dialog;
        this.sys_user_service = sys_user_service;
        this.dialogRef = dialogRef;
        this.data = data;
        //this.sys_chuyen_nganh = data;
        this.sys_chuyen_nganh_model = data;
        if (this.sys_chuyen_nganh_model.db.id == 0)
            this.Save();
    }
    Close() {
        this.dialogRef.close();
    }
    Save() {
        this.sys_chuyen_nganh_service.add(this.sys_chuyen_nganh_model).subscribe((result) => {
            var data = result;
            this.check_error = data.error;
            if (this.check_error.length === 0) {
                this.Close();
                Swal.fire({
                    icon: 'success',
                    title: 'Thành công',
                    showConfirmButton: false,
                    timer: 2000,
                }).then((result) => { });
            }
        });
    }
    Edit() {
        this.sys_chuyen_nganh_service
            .edit(this.sys_chuyen_nganh_model)
            .subscribe((result) => {
            var data = result;
            this.check_error = data.error;
            if (this.check_error.length === 0) {
                this.Close();
                Swal.fire({
                    icon: 'success',
                    title: 'Thành công',
                    showConfirmButton: false,
                    timer: 2000,
                }).then((result) => { });
            }
        });
    }
    ngOnInit() {
        this.lst_status = [
            {
                id: '1',
                name: 'Đang sử dụng',
            },
            {
                id: '2',
                name: 'Ngưng sử dụng',
            },
        ];
    }
};
sys_chuyen_nganh_popupComponent = __decorate([
    Component({
        selector: 'sys_chuyen_nganh_popup',
        templateUrl: './popupAdd.component.html',
        styleUrls: ['./popupAdd.component.scss'],
    }),
    __param(5, Inject(MAT_DIALOG_DATA)),
    __metadata("design:paramtypes", [HttpClient,
        sys_chuyen_nganh_service,
        MatDialog,
        sys_user_service,
        MatDialogRef,
        sys_chuyen_nganh_model])
], sys_chuyen_nganh_popupComponent);
export { sys_chuyen_nganh_popupComponent };
//# sourceMappingURL=popupAdd.component.js.map